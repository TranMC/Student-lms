class TeacherScores {
    ...
    saveScore() {
        ...
        if (this.validateScoreData(scoreData)) {
            if (scoreId) {
                ...
                if (index !== -1) {
                    this.scores[index] = { ...scoreData, id: scoreId };
                }
            } else {
                // Thêm điểm mới
                this.scores.push({
            ...
            this.loadScoreTable();
            this.closeModal();
            this.showToast(scoreId ? 'Đã cập nhật điểm thành công' : 'Đã thêm điểm thành công');
        }
    }
    ...
    editScore(scoreId) {
        ...
        if (score) {
            this.showAddScoreForm(score);
        } else {
            this.showToast('Không tìm thấy thông tin điểm', 'error');
        }
    }
    ...
    showToast(message, type = 'success') {
        ...
        toast.className = `fixed bottom-4 right-4 ${type === 'success' ? 'bg-green-500' : 'bg-red-500'} text-white px-4 py-2 rounded-lg shadow-lg z-50`;
    ...
    getStudentById(studentId) {
        return this.students.find(s => s.id === parseInt(studentId));
    }
